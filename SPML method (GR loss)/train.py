...
def run_train(P):
    ...
    with torch.set_grad_enabled(phase == 'train'):
        for batch in tqdm(dataloader[phase]):
            ...
            for i, pred in enumerate(preds_np):
                predictions.append({'idx': int(idx[i].item()), 'predictions': pred.tolist()})
            batch_stack += preds_np.shape[0]
    ...
    output_file = os.path.join(P['save_path'], 'test_predictions.json')
...
'split_seed': 42,
'val_frac': 0.1,
'num_classes': 18,  # Number of country labels
'bsize': 16,  # Batch size
'num_workers': 4,
'num_epochs': 1,
